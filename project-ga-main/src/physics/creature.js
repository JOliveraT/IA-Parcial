...
const HIP_MIN = deg(-60);
const HIP_MAX = deg(60);
...
export class Creature {
  ...
  createBody() {
    ...
    const tibiaCenterY = lowerLegBottomY - d.tibiaLength / 2;
    const kneeY = lowerLegBottomY - d.tibiaLength;
    const upperCenterY = kneeY - d.femurLength / 2;
    const hipY = kneeY - d.femurLength;
    const bodyY = hipY - d.bodyHeight / 2;

    const leftX = d.startX - d.stanceWidth / 2;
    const rightX = d.startX + d.stanceWidth / 2;
    ...
    this.leftThigh = Bodies.rectangle(leftX, upperCenterY, d.femurWidth, d.femurLength, { ...baseOpt, render: { fillStyle: '#355070' } });
    this.rightThigh = Bodies.rectangle(rightX, upperCenterY, d.femurWidth, d.femurLength, { ...baseOpt, render: { fillStyle: '#355070' } });
    this.leftCalf = Bodies.rectangle(leftX, tibiaCenterY, d.tibiaWidth, d.tibiaLength, { ...baseOpt, render: { fillStyle: '#8ecae6' } });
    this.rightCalf = Bodies.rectangle(rightX, tibiaCenterY, d.tibiaWidth, d.tibiaLength, { ...baseOpt, render: { fillStyle: '#8ecae6' } });

    this.constraints = [
      Constraint.create({ bodyA: this.torso, pointA: { x: -d.hipInset, y: d.bodyHeight / 2 - 1 }, bodyB: this.leftThigh, pointB: { x: 0, y: -d.femurLength / 2 }, stiffness: 0.98, length: 0, render: { visible: false } }),
      Constraint.create({ bodyA: this.torso, pointA: { x: d.hipInset, y: d.bodyHeight / 2 - 1 }, bodyB: this.rightThigh, pointB: { x: 0, y: -d.femurLength / 2 }, stiffness: 0.98, length: 0, render: { visible: false } }),
    ...
    Body.setAngle(this.leftThigh, deg(8));
    Body.setAngle(this.rightThigh, deg(-8));
    Body.setAngle(this.leftCalf, deg(14));
    Body.setAngle(this.rightCalf, deg(10));
  ...
  applyJointControl(part, base, target, minA, maxA, gain) {
    ...
    Body.setAngularVelocity(part, clamp(part.angularVelocity + delta, -3.5, 3.5));
